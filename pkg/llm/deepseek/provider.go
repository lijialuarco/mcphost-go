...
func (p *Provider) CreateMessage(
	...
	deepseekMessages := make([]MessageParam, 0, len(messages))
	...
	for _, msg := range messages {
		...
		if len(toolCalls) > 0 {
			...
			// Convert to deepseek tool calls format
		...
		deepseekMessages = append(deepseekMessages, param)
	}

	// Log the final message array
	log.Debug("sending messages to deepseek",
	...
	if prompt != "" {
		content := prompt
		deepseekMessages = append(deepseekMessages, MessageParam{
			Role:    "user",
			Content: &content,
		})
	}
